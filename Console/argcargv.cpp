...
wchar_t * _ppszArgv[_MAX_CMD_LINE_ARGS+1];
...
int __cdecl _ConvertCommandLineToArgcArgv(const wchar_t *lpszSysCmdLine)
{
	...
	int cbCmdLine = wcslen( lpszSysCmdLine );
	...
	pHeap = HeapAlloc( GetProcessHeap(), 0, cbCmdLine*sizeof(wchar_t)+16 );

	if ( !pHeap )
		return 0;

	wchar_t *pszCmdLine = (wchar_t *) pHeap;

	// Copy the system version of the command line into our copy
	wcscpy( pszCmdLine, lpszSysCmdLine );

	if ( L'"' == *pszCmdLine )   // If command line starts with a quote ("),
	{							// it's a quoted filename.	Skip to next quote.
		...
		if ( *pszCmdLine )		// Did we see a non-NULL ending?
			*pszCmdLine++ = L'\0';	// Null terminate and advance to next char
		else
	...
	else	// A regular (non-quoted) filename
	{
		...
		while ( *pszCmdLine && (L' ' != *pszCmdLine) && (L'\t' != *pszCmdLine) )
			pszCmdLine++;
	...
	while ( 1 )
	{
		...
		if ( L'\0' == *pszCmdLine ) // End of command line???
			return argc;

		if ( L'"' == *pszCmdLine )   // Argument starting with a quote???
		{
		...
		if ( argc >= (_MAX_CMD_LINE_ARGS) )
                {
                        LOG(TAG_CONSOLE "_ppszArgv=%s",_ppszArgv);
                       	return argc;
                }
	}
}
